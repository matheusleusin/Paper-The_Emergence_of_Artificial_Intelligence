...
write.csv2(mat_tech_AI_Final, file = "Data_IPC/Matrix_IPC.csv", row.names = TRUE)
...
write.csv2(reg_tech1, file = "Data_IPC/reg_tech_FirstPeriod.csv", row.names = F)
...
write.csv2(reg_tech2, file = "Data_IPC/reg_tech_SecondPeriod.csv", row.names = F)
...
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
write.csv2(tabledata2, file = "Data_IPC/reg_tech_ThirdPeriod.csv", row.names = F)
...
jpeg("Figures_IPC/IPC_all_option1.jpg", width = 14, height = 10, units = 'in', res = 200)
IPC_Total1
dev.off()
...
#and save the final file, so we can use it again in section 1.3. (around the line 330)
write.csv2(reg_tech1, file = "Data_IPC/reg_techAI_FirstPeriod.csv", row.names = F)
...
write.csv2(reg_tech2, file = "Data_IPC/reg_techAI_SecondPeriod.csv", row.names = F)
...
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
write.csv2(tabledata2, file = "Data_IPC/reg_techAI_ThirdPeriod.csv", row.names = F)
