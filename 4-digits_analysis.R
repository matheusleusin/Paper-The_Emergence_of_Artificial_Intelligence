...
write.csv2(Data1period, file = "Files_created_with_the_code/data/files_code_4-digits_analysis/Data1period_RCA.csv", row.names = F)
rm(mat_reg_tech1)
#1.2.Second Period ----
...
write.csv2(IPC_RCAs, file = "Files_created_with_the_code/data/files_code_4-digits_analysis/IPC_RCAs.csv", row.names = F)
...
#colour:
jpeg("Files_created_with_the_code/figures/regular_resolution_ALL_FIGURES_ARE_HERE/Fig6_General_versus_AI.jpeg", width = 14, height = 7, units = 'in', res = 300)
multiplot(FigGen_Colour, FigAI_Colour, cols=1)
dev.off()

#high:
jpeg("Files_created_with_the_code/figures/high_resolution/Fig6_General_versus_AIhigh.jpeg", width = 14, height = 7, units = 'in', res = 800)
multiplot(FigGen_Colour, FigAI_Colour, cols=1)
dev.off()
...
#test <- IPC_all_patents_FirstPeriod
...
#IPC_all_patents_FirstPeriod$ctry_code <- patents_AI_specific_1st$ctry_code[match(IPC_all_patents_FirstPeriod$appln_id, patents_AI_specific_1st$appln_id)]
...
reg_RCA1st <- mat_reg_tech1 %>% location.quotient(binary = F) %>% 
  as.data.frame() %>% 
  rownames_to_column("ctry_code") %>% 
  as_tibble() %>% 
  gather(key = "Subclass", value = "RCA", -ctry_code) %>%
...
reg_RCA2nd <- mat_reg_tech2 %>% location.quotient(binary = F) %>% 
  as.data.frame() %>% 
  rownames_to_column("ctry_code") %>% 
  as_tibble() %>% 
  gather(key = "Subclass", value = "RCA", -ctry_code) %>%
...
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
setwd("large_files")
IPC_all_patents_Part1 <- fread("All_patents_and_IPC_codes_Part1.csv", header = F)
names(IPC_all_patents_Part1) <- c("appln_id", "ctry_code", "ipc_class_symbol", "priority_year")
IPC_all_patents_Part1 <- IPC_all_patents_Part1[, c((-4))]
...
reg_RCA3rd <- mat_reg_tech3 %>% location.quotient(binary = F) %>% 
  as.data.frame() %>% 
  rownames_to_column("ctry_code") %>% 
  as_tibble() %>% 
  gather(key = "Subclass", value = "RCA", -ctry_code) %>%
...
#3.2.Calculate AI Matrix and generate figure----
#3.2.1.First period----
rm(list=ls())
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
patents_AI_specific_1st <- read.csv("other_files/IPCs_AI.csv", sep = ";", header = TRUE, dec=",")
