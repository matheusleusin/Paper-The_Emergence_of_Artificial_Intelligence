...
#Now we will load a big file containing all priorities and their related Nace codes published in or after 2004. This file has
#58,935,336 lines;
Nace_all_patents_Part1 <- fread("all data nace post 2004.csv", header = F)#, nrow = 18000000)
Nace_all_patents_Part2 <- fread("all data nace post 2004.csv", header = F, nrow = 100, skip = 18000000)
...
mat_tech_AI <- create_sparse_matrix(i = Nace_all_patents_Part1 %>% pull(appln_id),
...
add_matrices_1 <- function(matrix1, matrix2, matrix3, matrix4) {
  a <- list(matrix1, matrix2, matrix3, matrix4)
...
mat_tech_AI_Final <- add_matrices_1(mat_tech_AI, mat_tech_AI2)
...
as.matrix(matrix4, rownames=NULL, rownames.value=NULL)
